...
function prepareParam<const param extends AbiParameter>({
  ...
  if (param.type === 'address') {
    return encodeAddress(value as unknown as Address)
  }
  if (param.type === 'bool') {
...
function encodeAddress(value: Address): PreparedParam {
  if (!isAddress(value)) throw new InvalidAddressError({ address: value })

  const hexAddress = base32AddressToHex({ address: value })

  return { dynamic: false, encoded: padHex(hexAddress) }
}
